...
def read_trg_set():
    ...
    global trg_pattern
    trg_pattern = np.loadtxt('patternsetIndividual.txt')
    for j in range(p):
        for i in range (N):
...
def write_hebb_wt():
    global w 
    #mat = np.matrix(w) 
    np.savetxt("nistpatterhebb.txt",w,fmt='%.4f')
...
def read_test_set():
    global test_pattern
    activity = 0 
    test_pattern = np.loadtxt('patternsetIndividual.txt')
    for j in range(p):
        for i in range (N):
...
def save_Weights(p):
    global w
    name = 'weight' + str(p) + '.txt'
    np.savetxt(name,w,fmt='%.4f')

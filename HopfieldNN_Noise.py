...
def recallnew():
    ...
    for k in range (p):
        ...
        for l in range(78):
            m = random.randint(0,32767) % 784 
            if (temp_pattern[k][m] == 1):
                temp_pattern[k][m] = -1
            else:
                temp_pattern[k][m] = 1                  
        img = temp_pattern[k].reshape(28,28)
        plt.imshow(img, cmap="Greys")
        plt.show()  
...
#         for j in range(N):
#             for i in range(N):
#                 temp = temp + temp_pattern[k][i]*w[i][j]
#             if(temp > 0 ):
#                 recalloutput[k][j]=1
#             else:
#                 recalloutput[k][j]=-1
#             temp = 0.0
# =============================================================================
        img = recalloutput[k].reshape(28,28)
        plt.imshow(img, cmap="Greys")
        plt.show() 
        print('Hamming distance with 90% Noise :', np.count_nonzero(test_pattern[k] != recalloutput[k]))

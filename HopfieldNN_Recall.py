...
test_pattern=np.zeros((p,SIZE))
recalloutput=np.zeros((p,SIZE))
...
def recallnew():
    ...
    global test_pattern
    global w
    temp_pattern = np.zeros((MAX,SIZE))
    temp_pattern = np.copy(test_pattern)
    temp = 0.0
    for k in range (p):
        #Recall_SG(temp_pattern[k])
        recalloutput[k] = np.matmul(w,temp_pattern[k])
        recalloutput[recalloutput >= 0 ] = 1 
        recalloutput[recalloutput < 0 ] = -1
        temp = np.count_nonzero(test_pattern[k] != recalloutput[k])
        print("The hamming distance is ", temp)
        if(temp!=0):
            print("The recall is Fail")
            i=0
            global lamda
            global trg_pattern
            convergence = False 
            ...
            new = [SIZE]
            while (convergence == False  and  sweep < MAXIT):
                pat_conv = relax(temp_pattern[k],N,change)
                carry = carry and pat_conv
                i = (i + 1) % p
                if (i==0):
                    convergence = carry 
            ...
            recalloutput[recalloutput >= 0 ] = 1 
            recalloutput[recalloutput < 0 ] = -1
            print("The Hamming distance post Relax",np.count_nonzero(test_pattern[k] != recalloutput[k]))

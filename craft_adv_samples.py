...
def saveEventList(dir_name, file_name, event_list):
...
def craft_one_type(sess, model, X, Y, dataset, attack, batch_size):
    ...
    for i in range(50):
        image_1d = np.reshape(X_adv[i], 784)
        
        reshaped_list.append(image_1d)
        
    df_reshaped_patterns = pd.DataFrame(reshaped_list, index=None)
    
    df_reshaped_patterns.to_csv('adv_patterns.txt')

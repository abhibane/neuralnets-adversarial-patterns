...
if __name__ == '__main__':
    ...
    index_list = range(0, 49, 5)
    
    symb_array = text_array_float[index_list]
    
    max_bound = np.max(symb_array)
    min_bound = np.min(symb_array)
    
    print('Symbol Array Shape:', symb_array.shape)
    print('Max :', np.max(symb_array))
    print('Min :', np.min(symb_array))
    ...
    training_data = np.copy(symb_array)
...
# =============================================================================
    ...
    for idx in range(1, 11):
        idx_start = 1 * (idx - 1)
        idx_end = 1 * idx
        patt_idx_list = np.array(symb_array[idx_start:idx_end])
        ...
        while noise_ratio < max_noise:
            ...
            for src_idx in range(patt_idx_list.shape[0]):
                for sim_iter in range(sim_count):
                    ...
                    user = np.copy(symb_array[src_idx])
                    ...
                    for i in range(symb_array.shape[0]):
                        hamming_dist = np.count_nonzero(symb_array[i] != recalled_vector)
    ...
    fileStr = 'Symb_Misclassification_Noise_P' + str(symb_array.shape[0]) + '_RBM_h' + str(hidden_nodes) + '_Int' + str(interval_count)
